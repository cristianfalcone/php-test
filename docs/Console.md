...
Console::command('build', function () {
    ...
    if ($verbose) Console::info('Verbose mode enabled');
    Console::line('Files: ' . count($files));
    Console::success('Output: ' . $output);
    return 0;
})
...
```php
Console::command('deploy', function () {
    $o = Console::options();
    $env = $o['env'] ?? 'production';
    Console::info('Deploying {env}…', ['env' => $env]);
    return 0;
})
...
Access them inside your handler with `Console::options()`.
...
Console::warning('Cache miss');   // → stderr
Console::error('Failed to save'); // → stderr
```
...
```php
Console::timestamps(true)->colors(false);  // force “logger mode”
Console::timestamps(false)->colors(true);  // force “interactive mode”
```
...
```php
// Global
Console::use(function (?Throwable $error, callable $next) {
    $start = microtime(true);
    $code  = $next($error);
    Console::line(sprintf('T=%.2fs', microtime(true) - $start));
    return $code;
});
...
```php
Console::line(Console::bold('Important'));
Console::bold()->red()->line('Critical!');
Console::dim()->italic()->line('(optional note)');
```
...
```php
use Ajo\Core\Console as ConsoleCore;
...
Console::command('build', function () {
    ...
    if ($minify) Console::line('Minifying…');
    if ($sourcemap) Console::line('Sourcemaps…');
    return 0;
})
...
foreach ($files as $i => $f) {
    Console::dim()->line(sprintf('[%d/%d] %s', $i+1, count($files), $f));
}
...
try {
    ...
    Console::error('Failed saving order {id}', ['id' => $id, 'exception' => $e]);
}
```
...
Console implements the PSR‑3 logger interface (`Psr\Log\LoggerInterface`). You can:
...
```
Ajo\Console (Facade) ─┬─> static DX via __callStatic magic method
                      ├─> full IDE autocomplete via @method tags
                      └─> instance swapping for tests

Ajo\ConsoleCore      ─┬─> PSR-3 logger + CLI implementation
...
* Prefer the **facade** in application code (`Ajo\Console`).
* Use the **core instance** in tests or when you need custom streams (`Ajo\ConsoleCore`).

## Full API Reference

> The list below reflects the refactored surface (English naming, `line()` neutral output, PSR‑3 levels).

### Facade (static), `Ajo\Console`
...
* `Console::command(string $name, callable $handler): ConsoleCore`
* `Console::describe(string $text): ConsoleCore`
* `Console::usage(string $pattern): ConsoleCore`
* `Console::option(string $flags, string $description = '', mixed $default = null): ConsoleCore`
* `Console::example(string $usage, ?string $desc = null): ConsoleCore`
* `Console::use(callable|string $prefixOrHandler, ?callable $handler = null): ConsoleCore` — register middleware globally or for a prefix.

#### Execution & environment

* `Console::dispatch(?string $command = null, array $arguments = [], $stdout = null, $stderr = null): int`
* `Console::timestamps(bool $enable = true): ConsoleCore`
* `Console::colors(bool $enable = true): ConsoleCore`
* `Console::isInteractive(): bool`

#### Accessors

* `Console::bin(): string`
* `Console::arguments(): array`   — raw arguments (post‑command)
* `Console::options(): array`     — parsed options; positionals in `['_']`

#### Output

* `Console::line(string $message = ''): void` — neutral line → stdout
* `Console::success(string $message, array $context = []): void` — semantic success → stdout
* `Console::blank(int $lines = 1): void`
* `Console::write(string $message, $stream = null): void` — low‑level write (no labels)

#### PSR‑3 levels

* `Console::debug/info/notice/warning/error/critical/alert/emergency(string|\Stringable $message, array $context = []): void`
* `Console::log(string $level, string|\Stringable $message, array $context = []): void`
...
| Aspect / Tool          | Console (this project)                                                       | Symfony Console                                        | Laravel Artisan                                                  | Monolog                                                         |
| ---------------------- | ----------------------------------------------------------------------------- | ------------------------------------------------------ | ---------------------------------------------------------------- | --------------------------------------------------------------- |
