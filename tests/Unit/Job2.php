...
Test::describe('Job2 Unit Tests', function () {

    // ========================================================================
    // Cron Parsing
    // ========================================================================

    Test::it('parses 5 and 6 field cron expressions', function () {
        // 5 fields normalized to 6 (prepend 0) - this is done by Job2::cron()
        // but we test Cron2::parse() directly with 6 fields
        $cron5Normalized = Cron2::parse('0 */15 * * * *'); // Simulating what Job2::cron() does
    ...
    Test::it('supports ranges, steps, and lists in cron expressions', function () {
        // Range with step: 1-10/2 -> 1,3,5,7,9
        $cron = Cron2::parse('0 1-10/2 * * * *');
        Test::assertEquals([1, 3, 5, 7, 9], $cron['min']['list']);

        // List: 1,2,40
        $cron = Cron2::parse('0 1,2,40 * * * *');
        ...
        $cron = Cron2::parse('0 5,1,5,3,1 * * * *');
        Test::assertEquals([1, 3, 5], $cron['min']['list']);
    });

    Test::it('handles DOM vs DOW semantics correctly', function () {
        // Both * -> always match (when sec/min/hour/mon match)
        $cronBothStar = Cron2::parse('0 0 0 * * *');
        ...
        Test::assertTrue(Cron2::matches($cronBothStar, $timestamp));

        // Only DOM specified (DOW is *) -> match by DOM only
        $cronDom15 = Cron2::parse('0 0 0 15 * *');
        Test::assertFalse($cronDom15['domStar']);
        Test::assertTrue($cronDom15['dowStar']);
        Test::assertTrue(Cron2::matches($cronDom15, new DateTimeImmutable('2025-01-15 00:00:00')));
        Test::assertFalse(Cron2::matches($cronDom15, new DateTimeImmutable('2025-01-16 00:00:00')));

        // Only DOW specified (DOM is *) -> match by DOW only
        $cronWed = Cron2::parse('0 0 0 * * 3'); // Wednesday
        Test::assertTrue($cronWed['domStar']);
        Test::assertFalse($cronWed['dowStar']);
        Test::assertTrue(Cron2::matches($cronWed, new DateTimeImmutable('2025-01-15 00:00:00'))); // Wed
        Test::assertFalse(Cron2::matches($cronWed, new DateTimeImmutable('2025-01-16 00:00:00'))); // Thu

        // Both specified -> DOM OR DOW
        $cronBoth = Cron2::parse('0 0 0 1 * 1'); // 1st of month OR Monday
        Test::assertFalse($cronBoth['domStar']);
        Test::assertFalse($cronBoth['dowStar']);
        Test::assertTrue(Cron2::matches($cronBoth, new DateTimeImmutable('2025-01-01 00:00:00'))); // 1st (Thu)
        Test::assertTrue(Cron2::matches($cronBoth, new DateTimeImmutable('2025-01-06 00:00:00'))); // Monday (6th)
        Test::assertFalse(Cron2::matches($cronBoth, new DateTimeImmutable('2025-01-07 00:00:00'))); // 7th (Tue)
    });

    Test::it('advances nextMatch across month boundaries', function () {
        // Monthly: 0 0 0 1 * * (first day of each month at midnight)
        $cronMonthly = Cron2::parse('0 0 0 1 * *');

        $from = new DateTimeImmutable('2025-01-31 23:59:59');
        $next = Cron2::nextMatch($cronMonthly, $from);
        ...
        $cronNever = Cron2::parse('0 0 0 31 2 *'); // Feb 31 (never exists)

        try {
            Cron2::nextMatch($cronNever, new DateTimeImmutable('2025-01-01 00:00:00'));
    ...
    Test::it('matches with second precision', function () {
        $cron = Cron2::parse('*/5 * * * * *'); // Every 5 seconds

        Test::assertTrue(Cron2::matches($cron, new DateTimeImmutable('2025-01-15 10:30:00')));
        Test::assertTrue(Cron2::matches($cron, new DateTimeImmutable('2025-01-15 10:30:05')));
        Test::assertTrue(Cron2::matches($cron, new DateTimeImmutable('2025-01-15 10:30:10')));

        Test::assertFalse(Cron2::matches($cron, new DateTimeImmutable('2025-01-15 10:30:01')));
        Test::assertFalse(Cron2::matches($cron, new DateTimeImmutable('2025-01-15 10:30:03')));
    });

    Test::it('uses nextMatch to find next occurrence', function () {
        $cronEvery10Sec = Cron2::parse('*/10 * * * * *');

        $from = new DateTimeImmutable('2025-01-15 10:30:07');
        $next = Cron2::nextMatch($cronEvery10Sec, $from);

        Test::assertEquals('2025-01-15 10:30:10', $next->format('Y-m-d H:i:s'));

        // Next after that
        $next2 = Cron2::nextMatch($cronEvery10Sec, $next);
        Test::assertEquals('2025-01-15 10:30:20', $next2->format('Y-m-d H:i:s'));
    });

    Test::it('matches complex time patterns', function () {
        // Weekdays at 9am
        $cronWeekdays9am = Cron2::parse('0 0 9 * * 1-5');

        Test::assertTrue(Cron2::matches($cronWeekdays9am, new DateTimeImmutable('2025-01-13 09:00:00'))); // Mon
        Test::assertTrue(Cron2::matches($cronWeekdays9am, new DateTimeImmutable('2025-01-17 09:00:00'))); // Fri
        Test::assertFalse(Cron2::matches($cronWeekdays9am, new DateTimeImmutable('2025-01-18 09:00:00'))); // Sat
        Test::assertFalse(Cron2::matches($cronWeekdays9am, new DateTimeImmutable('2025-01-13 10:00:00'))); // Mon 10am
    });

    Test::it('handles edge cases in matching', function () {
        // Last second of the year
        $cronEverySecond = Cron2::parse('* * * * * *');
        Test::assertTrue(Cron2::matches($cronEverySecond, new DateTimeImmutable('2024-12-31 23:59:59')));

        // First second of the year
        Test::assertTrue(Cron2::matches($cronEverySecond, new DateTimeImmutable('2025-01-01 00:00:00')));

        // Leap year Feb 29
        $cronFeb29 = Cron2::parse('0 0 0 29 2 *');
        Test::assertTrue(Cron2::matches($cronFeb29, new DateTimeImmutable('2024-02-29 00:00:00'))); // 2024 is leap
        Test::assertFalse(Cron2::matches($cronFeb29, new DateTimeImmutable('2025-02-28 00:00:00'))); // 2025 not leap
    });
    ...
    Test::it('schedule same name twice switches focus', function () {
        $pdo = new \PDO('sqlite::memory:');
        $clock = new FakeClock(new DateTimeImmutable('2025-01-15 10:30:45'));
        $job = new Job2($pdo, $clock, 'jobs_test');
    ...
    Test::it('fluent modifiers override defaults', function () {
        $pdo = new \PDO('sqlite::memory:');
        $clock = new FakeClock();
        $job = new Job2($pdo, $clock, 'jobs_test');
    ...
    Test::it('cron helpers map to correct expressions', function () {
        $pdo = new \PDO('sqlite::memory:');
        $clock = new FakeClock();
        $job = new Job2($pdo, $clock, 'jobs_test');
    ...
    Test::it('time limiters modify cron fields', function () {
        $pdo = new \PDO('sqlite::memory:');
        $clock = new FakeClock();
        $job = new Job2($pdo, $clock, 'jobs_test');
    ...
    Test::it('backoff uses full jitter within bounds', function () {
        $pdo = new \PDO('sqlite::memory:');
        $clock = new FakeClock();
        $job = new Job2($pdo, $clock, 'jobs_test');
    ...
    Test::it('backoff respects cap limit', function () {
        $pdo = new \PDO('sqlite::memory:');
        $clock = new FakeClock();
        $job = new Job2($pdo, $clock, 'jobs_test');
    ...
    Test::it('backoff with jitter=none returns exact exponential delay', function () {
        $pdo = new \PDO('sqlite::memory:');
        $clock = new FakeClock();
        $job = new Job2($pdo, $clock, 'jobs_test');
