...
Test::suite('Facade', function () {
    ...
    Test::case('proxies static methods to root statics', function () {
        Test::assertSame('hola', StubFacade::greet());
    });

    Test::case('rejects inaccessible statics', function () {
        Test::expectException(BadMethodCallException::class, function () {
            StubFacade::hidden();
        });
    });

    Test::case('proxies instance methods to underlying instance', function () {
        Test::assertSame('pong', StubFacade::ping());
    });

    Test::case('does not expose private instance methods', function () {
        Test::expectException(BadMethodCallException::class, function () {
            StubFacade::secret();
        });
    });

    Test::case('falls back missing methods to __call when available', function () {
        Test::assertSame('custom', StubFacade::dynamic('custom'));
    });

    Test::case('throws exception when missing methods without __call', function () {
        Test::expectException(BadMethodCallException::class, function () {
            PlainFacade::missing();
        });
    });

    Test::case('replaces underlying instance on swap', function () {
    ...
    Test::case('rejects invalid instances', function () {
