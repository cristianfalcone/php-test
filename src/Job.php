...
final class JobCore
{
  ...
  public function __construct(PDO $pdo, ?Clock $clock = null, string $table = 'jobs')
  {
    $this->pdo   = $pdo;
  ...
  public function register(ConsoleCore $cli): self
  {
    $cli->command('jobs:install', fn() => $this->install())
      ->describe('Create the jobs table in the database');

    $cli->command('jobs:run', function () {
      $opts = Console::options();
      $batch = (int)($opts['batch'] ?? self::BATCH_LIMIT);
      return $this->run($batch);
    })
      ->describe('Execute due jobs once')
      ->usage('[options]')
      ->option('--batch', 'Maximum number of jobs to process', (string)self::BATCH_LIMIT)
      ->example('--batch=50', 'Process up to 50 jobs');

    $cli->command('jobs:work', function () {
      $opts = Console::options();
      $batch = (int)($opts['batch'] ?? self::BATCH_LIMIT);
      $sleep = (int)($opts['sleep'] ?? 200);
      return $this->work($batch, $sleep);
    })
      ->describe('Run job worker continuously')
      ->usage('[options]')
      ->option('--batch', 'Maximum jobs per iteration', (string)self::BATCH_LIMIT)
      ->option('--sleep', 'Sleep duration in milliseconds when idle', '200')
      ->example('--batch=50 --sleep=500', 'Process 50 jobs per batch, sleep 500ms when idle');

    $cli->command('jobs:prune', function () {
      $opts = Console::options();
      $olderThan = (int)($opts['older-than'] ?? 86400);
      return $this->prune($olderThan);
    })
      ->describe('Remove stale locked jobs')
      ->usage('[options]')
      ->option('--older-than', 'Remove jobs locked longer than N seconds ago', '86400')
      ->example('--older-than=3600', 'Remove jobs locked more than 1 hour ago');
...
SQL);
  ...
  public function __call(string $name, array $args): self
  {
    ...
    if ($idx !== null) {
      ...
      $spec['parsed'] = Cron::parse($spec['cron']);
      return $this;
    }
  ...
  public function work(int $batch = self::BATCH_LIMIT, int $sleep = 200)
  {
    $this->running = true;
  ...
  private function enqueue(): void
  {
    ...
    foreach ($this->jobs as $name => $job) {
      ...
      $cron = $job['parsed'] ?? Cron::parse($job['cron']);

      if (Cron::matches($cron, $now)) $exec($now);

      $exec(Cron::nextMatch($cron, $now));
    }
  }
  ...
  private function execute(array $run): void
  {
    ...
    try {
      ...
      // Chain before() hooks: cada uno recibe el resultado del anterior
      $value = $args;
      foreach ($job['before'] as $fn) {
        $value = $fn($value) ?? $value;
      }

      // Resolver y ejecutar la task con el valor transformado por before()
      ...
      if (isset($job['task']) && is_callable($job['task'])) {
        ...
        if (is_callable([$class, 'handle'])) {
          ...
          $value = $obj->handle($value);
        }
      } else {
        throw new RuntimeException("No handler for job '{$run['name']}'");
      }

      // Chain then() hooks: cada uno recibe el resultado del anterior
      foreach ($job['then'] as $fn) {
        $value = $fn($value) ?? $value;
      }
      ...
      // Chain catch() hooks: pueden transformar error o recuperar (return value)
      foreach ($job['catch'] as $fn) {
        try {
          $recovered = $fn($error);
          // Si catch() retorna un valor (no null), consideramos que recuperó
          if ($recovered !== null) {
            $error = null; // Marca como recuperado
            break; // Salir del chain de catch al recuperar
          }
        } catch (Throwable $newError) {
          $error = $newError; // Re-throw o nuevo error
        }
      }

      // Si no hubo recuperación, manejar retry/delete
      if ($error !== null) {
        ...
        // Recuperado por catch(): borrar la fila
        $stmt = $this->pdo->prepare("DELETE FROM `{$table}` WHERE id=:id");
        $stmt->execute([':id' => $run['id']]);
      }
    } finally {
      // finally() no recibe parámetros (como en JS)
      foreach ($job['finally'] as $fn) $fn();
      $this->release($run['locks']);
    }
  }
...
// Job Facade — Static interface ----------------------------------------------

namespace Ajo;

/**
 * Job — Static facade for JobCore.
 *
 * Usage:
 *   Job::schedule('emails.send', fn() => sendEmails())
 *      ->everyFiveMinutes()
 *      ->queue('emails');
 *   
 *   Job::dispatch('emails.send');
 *   Job::run();
 *
 * For testing, use JobCore directly:
 *   $job = new JobCore($pdo);
 *   $job->schedule('test', fn() => doWork());
 */
final class Job
{
  use Facade;
