...
/**
 * ConsoleCore — CLI + PSR‑3 Logger (implementation class).
 *
 * This is the concrete implementation. Use Console2 facade for static access.
 * Use this class directly in tests: $cli = new ConsoleCore();
 *
 ...
 */
final class ConsoleCore extends Router implements LoggerInterface
{
  ...
  /**
   * Return registered commands (for testing/introspection).
   *
   * @return array<string, array{name:string,handler:callable,...}>
   */
  ...
  public function table(array $columns, array $rows): void
  {
    ...
    foreach ($rows as $row) {
      $this->line($renderRow(array_map(fn($key) => (string)($row[$key] ?? '-'), $keys)));
    }
  }
  ...
  public function progress(int $current, int $total, string $label = '', array $breakdown = [])
  {
    ...
    // Only use in-place updates when output is to an interactive TTY
    $inPlace = $this->isInteractive();
    $this->write($line . ($inPlace ? '' : PHP_EOL), null, $inPlace);
  }
  ...
  public function write(string $message, $stream = null, bool $inPlace = false): void
  {
    $stream ??= $this->stdout;
  ...
  private function emit(?string $level, string $message, array $context, $stream): void
  {
    ...
    // Auto-detect TTY settings if not explicitly set (lazy detection)
    // Save original values to restore later
    $originalColors = $this->colorsEnabled;
    $originalTimestamps = $this->timestampsEnabled;

    $this->colorsEnabled ??= $this->isTty($stream);
    $this->timestampsEnabled ??= !$this->isTty($stream);
    ...
    $this->colorsEnabled = $originalColors;
    $this->timestampsEnabled = $originalTimestamps;
  }
...
final class StyleBuilder
{
  /** @var list<string> */
  private array $stack;
  private ConsoleCore $console;

  public function __construct(ConsoleCore $console, array $stack)
  {
...
/* ================================================
 *  Console Facade — Static interface
 * ================================================ */
 ...
 *
 * Usage:
 *   Console::command('hello', fn() => (Console::success('Hello\!') || 0));
 *   exit(Console::dispatch());
 *
 * For testing, use ConsoleCore directly:
 *   $cli = new ConsoleCore();
